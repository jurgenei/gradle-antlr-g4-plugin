...
public class G4toClassTaskFunctionalTest {
    ...
    @Test
    public void convertsExplicitInputOutput() throws Exception {
        ...
        writeBuildFile(projectDir, """
                ...
                tasks.register('convertOne', name.jurgenei.gradle.xml.G4toClassTask) {
        ...
        final BuildResult result = run(projectDir, "convertOne");

        final BuildTask task = result.task(":convertOne");
        Assert.assertNotNull(task);
        Assert.assertEquals(TaskOutcome.SUCCESS, task.getOutcome());

        Assert.assertTrue(new File(projectDir, "build/out/mini.classes.sexp").isFile());
        Assert.assertTrue(new File(projectDir, "build/out/mini.model.sexp").isFile());
    }

    @Test
    public void failsWhenSingleFileModeIncomplete() throws Exception {
        final File projectDir = temporaryFolder.newFolder("functional-g4-to-class-invalid");
        writeSettings(projectDir);
        writeBuildFile(projectDir, """
                plugins {
                    id 'java'
                ...
                tasks.register('broken', name.jurgenei.gradle.xml.G4toClassTask) {
                    input 'src/main/antlr/mini.g4'
                }
                """);
        ...
        final BuildResult result = runAndFail(projectDir, "broken");
        Assert.assertTrue(result.getOutput().contains("Both inputFile and outputFile must be set together"));
    }
